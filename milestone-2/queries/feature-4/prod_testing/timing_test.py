...
def run_lap_info_query(cursor, conn, rID, lapNumber):
    """Run the lap info query and return execution time"""
    query_template = """
...
"""
    query = query_template.format(rID=rID, lapNumber=lapNumber)
...
def main():
    ...
    test_params = {"rID": 1116, "lapNumber": 10}

    iterations = 100
    results = {}

    print("Feature-4 Production Testing")
    print("=" * 50)
    print(
        f"Test parameters: rID={test_params['rID']}, lapNumber={test_params['lapNumber']}"
    )
    ...
    for opt_num in range(6):
        ...
        for i in range(iterations):
            ...
            exec_time = run_lap_info_query(cursor, conn, **test_params)
    ...
    with open(results_path, "w") as f:
        f.write("Feature-4 Timing Results\n")
        f.write("=" * 50 + "\n")
        f.write(
            f"Test parameters: rID={test_params['rID']}, lapNumber={test_params['lapNumber']}\n"
        )
