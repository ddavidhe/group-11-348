...
def run_average_lap_query(cursor, conn, rID, dID):
    """Run the average lap query and return execution time"""
    query = f"SELECT AVG(time) FROM laps WHERE rID = {rID} AND dID = {dID}"
...
def main():
    ...
    print("Feature-2 Production Testing")
    print("=" * 50)
    print(f"Test parameters: rID={test_params['rID']}, dID={test_params['dID']}")
    ...
    for opt_num in range(6):
        ...
        for i in range(iterations):
            ...
            exec_time = run_average_lap_query(cursor, conn, **test_params)
    ...
    with open(results_path, "w") as f:
        f.write("Feature-2 Timing Results\n")
        f.write("=" * 50 + "\n")
        f.write(
            f"Test parameters: rID={test_params['rID']}, dID={test_params['dID']}\n"
        )
